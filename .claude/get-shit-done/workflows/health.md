...
TASKS_DIR="/home/seven/data/coding/projects/seven/advisor/.claude/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in /home/seven/data/coding/projects/seven/advisor/.claude/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf /home/seven/data/coding/projects/seven/advisor/.claude/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
